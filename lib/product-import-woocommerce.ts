...
// Import all products from WooCommerce via server API
export const importAllProductsFromWooCommerce = async (): Promise<{
  success: boolean;
  ...
  try {
    ...
    const response = await fetch(`${serverUrl}/api/woocommerce/import-products`, {
      method: 'POST',
      headers: {
...
export const getAllProducts = async (): Promise<{
  ...
  try {
    const serverUrl = getServerUrl();
    const response = await fetch(`${serverUrl}/api/woocommerce/products`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
...
export const getSyncStats = async (): Promise<{
  ...
  try {
    const serverUrl = getServerUrl();
    const response = await fetch(`${serverUrl}/api/woocommerce/sync-status`, {
      method: 'GET',
      headers: {
        'Content-Type': 'application/json',
      },
    ...
    return {
      success: true,
      data: {
        lastSyncWooCommerce: result.lastWooCommerceSync ? new Date(result.lastWooCommerceSync) : undefined,
  ...
  try {
    ...
    return {
      ...
      data: {
        ...
        lastSyncWooCommerce: syncData?.lastSyncWooCommerce,
